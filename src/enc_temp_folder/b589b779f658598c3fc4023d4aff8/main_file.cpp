...
void place_piece(glm::mat4 model, ShaderProgram* shader, std::string piece, bool white, glm::vec2 pos) {
	...
	if (!white)
		M_temp = glm::rotate(M_temp, PI, glm::vec3(0.0f, 1.0f, 0.0f));
...
void drawScene(GLFWwindow* window,float angle_x,float angle_y) {
	...
	place_piece(M_start, sp, "king", "white_king", glm::vec2(-2, 2));

	{
		glm::mat4 M_temp = glm::translate(M_start, glm::vec3(-1.0f, 0.0f, 1.0f));
		M_temp = glm::scale(M_temp, glm::vec3(2.0f));
		glUniformMatrix4fv(sp->u("M"), 1, false, glm::value_ptr(M_temp));

		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textures.at("black_pawn"));
		glUniform1i(sp->u("tex"), 0);

		GameObject current_obj = objects.at("pawn");
		current_obj.BindAll(sp);
		glDrawArrays(GL_TRIANGLES, 0, current_obj.vertex_count);
	}
